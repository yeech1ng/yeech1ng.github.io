...
plot_ly(kreviews, 
        x = ~acting_cast, y = ~overall,
        type = 'scatter', mode = 'markers',
        color = ~overall,
        colors = palette,
        marker = list(size = 8)) %>%
  layout(title = "Relationship between Music Ratings and Overall Ratings",
         xaxis = list(title = "Acting Cast"),
         yaxis = list(title = "Overall Ratings"))
...
set.seed(123)
kdrama_reviews <- data.frame(
  title = paste0("K-drama", 1:100),
  story = runif(100, min = 1, max = 10),
  acting_cast = runif(100, min = 1, max = 10),
  music = runif(100, min = 1, max = 10),
  rewatch_value = runif(100, min = 1, max = 10),
  overall = runif(100, min = 1, max = 10)
)

# Compute correlation matrix
correlation_matrix <- cor(select(kdrama_reviews, -title))
