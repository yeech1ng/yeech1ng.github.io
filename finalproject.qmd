...
# Calculate average ratings for each language category
average_ratings <- rbind(
  kreviews %>% summarise(avg_rating_story = mean(story)),
  treviews %>% summarise(avg_rating_story = mean(story)),
  jreviews %>% summarise(avg_rating_story = mean(story))
)

# Add language column
average_ratings$language <- c("Korean", "Thai", "Japanese")

# Define color palette using colorbrewer
palette <- brewer.pal(n = 3, name = "Set2")

# Create interactive plot with plotly
plot_ly(average_ratings, x = ~language, y = ~avg_rating_story, type = 'bar', 
        marker = list(color = palette)) %>%
  layout(title = "Average User Ratings on Drama's Story",
         xaxis = list(title = "Language"),
         yaxis = list(title = "Average Rating on Story"),
         showlegend = FALSE)
 })
})
...
prop_data <- merged_data_produced %>%
  group_by(year, language) %>%
  summarize(prop_dramas = n() / sum(n())) %>%
  ungroup()

# Spread data to create separate columns for each language
spread_data <- prop_data %>%
  spread(key = language, value = prop_dramas, fill = 0)

# Calculate total number of dramas produced each year
total_dramas <- merged_data_produced %>%
  group_by(year) %>%
  summarise(total_dramas = n()) %>%
  ungroup()

# Join total dramas data with spread data
final_data <- spread_data %>%
  left_join(total_dramas, by = "year")

# Define color palette
palette <- brewer.pal(n = 3, name = "Set2")

# Plot with ggplot2
plot <- ggplot(final_data, aes(x = year)) +
  geom_bar(aes(y = total_dramas, fill = language), stat = "identity") +
  labs(title = "Yearly Production of Korean, Thai, and Japanese Dramas",
       x = "Year",
       y = "Total Number of Dramas Produced",
       fill = "Language") +
  scale_fill_manual(values = palette) +
  theme_bw()
...
# Add language column to each dataset
...
scatter_plot <- plot_ly(all_reviews, x = ~acting_cast, y = ~overall, color = ~language, colors = brewer.pal(3, "Set2")) %>%
  add_markers() %>%
  layout(title = "Correlation between Acting Cast Ratings and Overall Ratings",
         xaxis = list(title = "Acting Cast Ratings"),
         yaxis = list(title = "Overall Ratings"))

# Show the interactive plot
scatter_plot
cor(all_reviews$acting_cast, all_reviews$overall)
ggplot(all_reviews, aes(x = acting_cast, y = overall)) +
  geom_jitter() +
  geom_smooth()
```
...
# Plot scatter plot using Plotly
scatter_plot <- plot_ly(all_reviews, x = ~music, y = ~rewatch_value, color = ~language, colors = brewer.pal(3, "Set2")) %>%
  add_markers() %>%
  layout(title = "Correlation between Music Ratings and Rewatch Value",
         xaxis = list(title = "Music Ratings"),
         yaxis = list(title = "Rewatch Value"))
